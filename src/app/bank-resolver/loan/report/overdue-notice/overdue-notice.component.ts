      ...
      // this.svc.addUpdDel('Loan/GetDemandList',dt).subscribe(data=>{console.log(data)
        ...
        if(this.reportData2){
          this.reportData2.forEach(p => {
            debugger
            if(this.sys.ardbCD=="20" || this.sys.ardbCD=="3" ||this.sys.ardbCD=="26" ){
...
AmountToBengaliWords(amount: number): string {
  ...
  amount = Math.floor(amount); // ignore decimal for now
  ...
  for (const unit of units) {
    ...
    if (unitValue > 0) {
      result += getTwoDigitWord(unitValue) + ' ' + unit.name + ' ';
      amount %= unit.value;
    }
  }

  if (amount > 0) {
    result += getTwoDigitWord(amount);
  }

  return result.trim();

  function getTwoDigitWord(n: number): string {
    if (n <= 99) {
      return bengaliNumbers[n] || '';
    } else {
      const hundreds = Math.floor(n / 100);
      const rest = n % 100;
      let words = '';
      if (hundreds > 0) {
        words += bengaliNumbers[hundreds] + ' শত ';
      }
      if (rest > 0) {
        words += bengaliNumbers[rest] || '';
      }
      return words.trim();
    }
  }
