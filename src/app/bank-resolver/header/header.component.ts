...
// this.openModalMessage(this.Notice)
  ...
  async SubmitReport() {
      ...
      this.svc.addUpdDel<any>('Deposit/PopulateNearMatDetails',dt).subscribe(data=>{console.log(data)
        this.maturedData=data;
        if(!this.maturedData){
          this.count=0
          this.tot_prn=0
          this.tot_intt=0
          ...
          this.maturedData.forEach(e=>{
            this.tot_prn+=e.prn_amt;
            this.tot_intt+=e.intt_amt
          })
          this.count=this.maturedData.length;
          this.isLoading=false
        }
        },
        err => {
  ...
  toggleBadgeVisibility(){
        ...
        this.modalRef = this.modalService.show(this.template3, {class: 'modal-lg modal-dialog-centered'});
